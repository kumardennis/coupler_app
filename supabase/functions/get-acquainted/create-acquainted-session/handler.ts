...
import { getMondayDate } from "../../_shared/getNextMonday.ts";
...
export const handler = async (req: Request) => {
  ...
  try {
    const { coupleId, questionId } = await req
      .json();
    ...
    const availableSince = getMondayDate();

    const formattedDate = dayjs(availableSince).format("YYYY-MM-DD");

    const { data, error } = await supabase.from(
      "acquainted_sessions",
    ).insert({
      coupleId,
      acquaintedQuestionId: questionId,
      availableSince: formattedDate,
    }).select();
